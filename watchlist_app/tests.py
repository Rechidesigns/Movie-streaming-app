...
class StreamPlatformTestCase(APITestCase):
    ...
    def test_streamplatform_create(self):
        ...
        response = self.client.post(reverse('add-streaming-platform'), data)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    
    def test_streamplatform_list(self):
        response = self.client.get(reverse('add-streaming-platform'))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    
    def test_streamplatform_ind(self):
        response = self.client.get(reverse('streaming-detail', args=(self.stream.id,)))
...
class WatchListTestCase(APITestCase):
    ...
    def test_watchlist_ind(self):
        response = self.client.get(reverse('Movies-detail', args=(self.watchlist.id,)))
        self.assertEqual(response.status_code, status.HTTP_302_FOUND)
...
class ReviewTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.watchlist2 = models.WatchList.objects.create(platform=self.stream, title="Example Movie",
                                storyline="Example Movie", active=True)
        self.review = models.Review.objects.create(user_review=self.user, rating=5, description="Great Movie", 
